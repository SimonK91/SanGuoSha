	...
	while(running) //medans programmet körs
	{
		while( SDL_PollEvent( &event)) //så länge som det finns en event
		{
			...
			for(unsigned i = 0 ; i < all_objects.size() ; ++i)  //för varje objekt som finns i gamestatet
			{
				...
				{
					std::string command = dynamic_cast<active_object*>(all_objects.at(i))->handle_event(event); //kör handle event på objektet (detta ser om kriterier är uppfyllda för att göra något
				...
				{
					std::string command = dynamic_cast<window*>(all_objects.at(i))->handle_event(event); //kör handle event på objektet (detta ser om kriterier är uppfyllda för att göra något
					if(command != "") //om man fick tillbaka annat än en tom sträng
					{
						run_command(command,i); //kör kommandot för denna sträng
						//rensa events 

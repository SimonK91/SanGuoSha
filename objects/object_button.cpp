...
button::button
(const std::string& name, int x, int y, const std::string& command_,const std::string& image, int width, int height)
//:box.x(x),box.y(y),box.w(width),box.h(height),active(0)
{
	...
	apply_surface((width-(textSurface->w))/2 -5 ,8           ,textSurface,buttonSheet); //text på alla delar av knappen
	apply_surface((width-(textSurface->w))/2 -5 ,8+height    ,textSurface,buttonSheet);
